...
const createPart = async () => {
  ...
  /**
   * Crea los archivos CSS, PHP dentro del 'part'
   */
  const filesToCreate = ['style.css', 'template.php'];
  ...
  // 2. Crea los archivos CSS dentro de scss/parts/NUEVA_CATEGORIA/NUEVO_NAME
  ...
  /**
   * Crea los archivos JS dentro del 'private/js/part/...'
   */
  // Dentro de 'private/js/parts':
  const basePrivateJsDir = path.join(__dirname, 'private', 'js', 'parts');
  const jsDirectory = path.join(basePrivateJsDir, selectedCategory, partName);
  // Crea la estructura de directorios si no existe
  fs.mkdirSync(jsDirectory, { recursive: true });
  // Crea el archivo script.js dentro del directorio
  fs.writeFileSync(path.join(jsDirectory, 'script.js'), '', 'utf-8');
