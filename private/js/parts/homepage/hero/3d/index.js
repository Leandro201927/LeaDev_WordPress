...
import { registerComponent } from '../../../../global/utils'
import { AccordionController } from '../../../../global/controllers/accordion-controller'
import { SliderController, SliderImageController } from '../../../../global/controllers/slider-controller'
import RenderLoaderScreen from '../../../../global/3d/loader-handler'

export default async function Homepage3DScript() {
  const loaderHandler = RenderLoaderScreen(document.querySelector('canvas#webgl1'), 'center')
  ...
  registerComponent(async () => {
    ...
    function prepareScene2() {
      ...
      /**
       * Laptop screen resources load
       */
      // Movier
      const movierVideo = document.createElement('video');
      movierVideo.src = `${templateUrl}/assets/3d/homepage/movier.mp4`;
      movierVideo.crossOrigin = 'anonymous'
      movierVideo.muted = true;
      const movierTextureScreen = new THREE.VideoTexture(movierVideo);
      // CPROC
      const cprocImage = document.getElementById('cproc-img');
      const cprocTextureScreen = new THREE.TextureLoader().load(cprocImage.dataset.src);
      ...
      function projectImgOrVideoInLaptopScreen(company) {
        ...
        let texture;
        if(company === 'movier') {
          actualLaptopScreenElement = movierVideo
          texture = movierTextureScreen
          updateRepetirControl(false) // show 'Repetir' control mesh

          movierVideo.play();
        } else if('cproc') {
          actualLaptopScreenElement = cprocImage
          texture = cprocTextureScreen
          updateRepetirControl(true) // hide 'Repetir' control mesh
        }
